...
import {
  ...
  ContractListFilters,
  ContractListSort,
} from "../services/contractHistory.service.js";
...
export const getContractListHandler = async (
  ...
  try {
    ...
    if (req.query.uploadedAfter)
      filters.uploadedAfter = new Date(String(req.query.uploadedAfter));
    if (req.query.uploadedBefore)
      filters.uploadedBefore = new Date(String(req.query.uploadedBefore));
    if (req.query.status)
    ...
    next(
      error instanceof AppError
        ...
        : new AppError(
            500,
            `Failed to get contract list: ${error instanceof Error ? error.message : "Unknown error"}`,
          ),
    );
  }
};
...
export const getContractDetailHandler = async (
  ...
  try {
    ...
    const result = await contractHistoryService.getContractDetail(
      contractId,
      userId,
    );
    ...
    next(
      error instanceof AppError
        ...
        : new AppError(
            500,
            `Failed to get contract detail: ${error instanceof Error ? error.message : "Unknown error"}`,
          ),
    );
  }
};
...
export const deleteContractHandler = async (
  ...
  try {
    ...
    const deleted = await contractHistoryService.softDeleteContract(
      contractId,
      userId,
    );
