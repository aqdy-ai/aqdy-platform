...
const EVALUATION_SUITE: EvaluationTestCase[] = [
  {
    ...
    queryText:
      "The company will be responsible for all direct and indirect damages without any cap on liability.",
  ...
  },
  {
    targetClauseId: "clause_001_unlimited_liability",
    queryText:
      "يتحمل الطرف الثاني كامل المسؤولية عن الأضرار بدون حد أقصى للتعويضات المالية.",
  ...
  },
  {
    targetClauseId: "clause_002_perpetual_confidentiality",
    queryText:
      "تبقى التزامات السرية سارية إلى الأبد دون تحديد مدة انتهاء للالتزام.",
  ...
  },
  {
    ...
    queryText:
      "You are banned from working for any competitor anywhere for five years.",
  ...
  },
  {
    targetClauseId: "clause_003_noncompete_5years",
    queryText:
      "لا يحق للموظف العمل لدى أي منافس لمدة خمس سنوات كاملة بعد انتهاء الخدمة.",
  ...
  },
  {
    ...
    queryText:
      "Employee has to work extra hours on weekends and public holidays without any overtime pay.",
  ...
  },
  {
    ...
    queryText:
      "يجوز لصاحب العمل تمديد فترة التجربة لتصبح ستة أشهر بدلاً من ثلاثة دون موافقة الموظف.",
...
async function runEvaluation() {
  ...
  console.log("==============================================================");
  console.log(
    `Suite Size: ${EVALUATION_SUITE.length} test cases (English/Arabic mix)`,
  );
  ...
  for (let i = 0; i < EVALUATION_SUITE.length; i++) {
    ...
    console.log(
      `[Test ${i + 1}/${EVALUATION_SUITE.length}] Evaluating: "${testCase.description}"`,
    );
    ...
    console.log(
      `   ⚡ Latency: ${duration}ms | Target Matched: ${isMatched ? `✅ (Rank ${targetIdx + 1})` : "❌ Not Found"}`,
    );
    console.log(`   Top 3 retrieved:`);
    searchResults.slice(0, 3).forEach((r, idx) => {
      console.log(
        `      ${idx + 1}. [${r.id}] (Score: ${r.score.toFixed(4)}, Cat: ${r.category}, Risk: ${r.riskLevel})`,
      );
    });
    console.log("");
  ...
  console.log(
    `Recall@5 Accuracy         : ${recallAt5.toFixed(2)}% (${matchesFoundCount}/${EVALUATION_SUITE.length})`,
  );
  console.log("--------------------------------------------------------------");
  console.log(
    "| Test Case Description | Target Clause ID | Matched? | Rank | Latency |",
  );
  console.log(
    "|-----------------------|------------------|----------|------|---------|",
  );
  results.forEach((r) => {
    console.log(
      `| ${r.case.padEnd(21)} | ${r.target.padEnd(16)} | ${r.matched ? "✅ Yes " : "❌ No  "} | ${r.rank !== -1 ? r.rank.toString().padEnd(4) : "N/A "} | ${r.latency.toString().padEnd(5)} ms |`,
    );
  });
  console.log(
    "==============================================================\n",
