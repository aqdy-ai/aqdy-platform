...
export class ContractExportService {
    ...
    async getExportData(userId: string): Promise<ExportRow[]> {
        ...
        return contracts.map(contract => {
            ...
            return {
                ...
                analysisDate: analysis ? (analysis as any).createdAt?.toISOString() ?? '' : '',
                overallRisk: analysis?.executiveSummary?.overallRisk ?? 'pending',
                criticalClauses: clauses.filter(c => c.riskLevel === 'critical').length,
    ...
    async generateJSON(userId: string): Promise<ExportResult> {
        ...
        return {
            data: JSON.stringify({ exportedAt: new Date(), contracts: exportData }, null, 2),
