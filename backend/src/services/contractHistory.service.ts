...
export class ContractHistoryService {
    ...
    async getContractList(
        ...
        const dbSortField = sort.field === 'uploadedAt' ? 'uploadedAt' : 'uploadedAt';
        const dbSortOrder = sort.field === 'uploadedAt' && sort.order === 'asc' ? 1 : -1;
        ...
        if (sort.field === 'analyzedAt') {
            contractItems.sort((a, b) => {
                ...
                const dateA = analysisA?.createdAt ? new Date(analysisA.createdAt).getTime() : 0;
                const dateB = analysisB?.createdAt ? new Date(analysisB.createdAt).getTime() : 0;
