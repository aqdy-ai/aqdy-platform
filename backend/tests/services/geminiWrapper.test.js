...
describe("Gemini Wrapper", () => {
    ...
    describe("call() — error handling", () => {
        test("should throw on non-string response", async () => {
            ...
            await expect(geminiWrapper.call("Numeric response")).rejects.toThrow("Unexpected non-string response from Gemini");
        });
        test("should propagate errors from the underlying client", async () => {
            ...
            await expect(geminiWrapper.call("Over quota")).rejects.toThrow("API quota exceeded");
        });
    });
});
//# sourceMappingURL=geminiWrapper.test.js.map