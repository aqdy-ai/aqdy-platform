...
describe("PdfService - File Validation", () => {
    test("should reject non-PDF files", () => {
        ...
        expect(() => pdfService.validateFile(file)).toThrow("Invalid file type. Only PDF files are allowed.");
    });
    test("should reject files over 10MB", () => {
        const file = createMockFile("application/pdf", 11 * 1024 * 1024);
        expect(() => pdfService.validateFile(file)).toThrow("File size exceeds 10MB limit.");
    });
...
describe("DocxService - File Validation", () => {
    test("should reject non-DOCX files", () => {
        const file = createMockFile("image/png", 1024);
        expect(() => docxService.validateFile(file)).toThrow("Invalid file type. Only DOCX/DOC files are allowed.");
    });
    test("should reject files over 10MB", () => {
        const file = createMockFile("application/vnd.openxmlformats-officedocument.wordprocessingml.document", 11 * 1024 * 1024);
        expect(() => docxService.validateFile(file)).toThrow("File size exceeds 10MB limit.");
    });
    test("should accept valid DOCX file", () => {
        const file = createMockFile("application/vnd.openxmlformats-officedocument.wordprocessingml.document", 1024);
...
//# sourceMappingURL=parsing.service.test.js.map