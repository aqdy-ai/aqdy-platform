...
const createMatch = (id, score, category, riskLevel) => ({
    id,
    score,
    category,
    riskLevel,
...
describe("RAG Accuracy: MMR Diversity", () => {
    ...
    test("should return max 3 results", () => {
        const matches = Array.from({ length: 10 }, (_, i) => createMatch(`c${i}`, 0.9 - i * 0.05, "liability", "high"));
        const result = ragService.applyMMR(matches);
...
//# sourceMappingURL=rag.accuracy.test.js.map