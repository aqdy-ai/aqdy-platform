...
const mockCallFallback = jest.fn();
...
const createPineconeHit = (id, score, fields = {}) => ({
    _id: id,
    _score: score,
    fields: {
        category: "liability",
...
describe("RAG Service — search()", () => {
    ...
    test("should handle search errors and log them", async () => {
        ...
        await expect(ragService.search("test", { enableQueryExpansion: false })).rejects.toThrow("RAG search operation failed");
    });
});
