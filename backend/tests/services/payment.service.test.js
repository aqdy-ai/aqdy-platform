...
describe("PaymentService Unit Tests", () => {
    ...
    describe("createCheckoutSession", () => {
        ...
        it("should create a Stripe customer if one does not exist for the user", async () => {
            ...
            jest
                ...
                .mockResolvedValue({ url: "https://stripe.com/session" });
    ...
    describe("confirmSession", () => {
        it("should fulfill subscription and return balance for paid status", async () => {
            ...
            jest
                ...
                .mockResolvedValue(mockSubDetails);
            Subscription.create.mockResolvedValue({
                _id: "sub_local_1",
            });
            creditsService.getBalance.mockResolvedValue(100);
        ...
        it("should return pending status if payment is not paid", async () => {
            jest
                .spyOn(stripe.checkout.sessions, "retrieve")
                .mockResolvedValue({ payment_status: "unpaid" });
    ...
    describe("handleWebhook", () => {
        it("should skip already processed events to ensure idempotency", async () => {
            ...
            AuditLog.findOne.mockResolvedValue({
                id: "log_1",
            });
        ...
        it("should handle customer.subscription.deleted event and downgrade user", async () => {
            ...
            Subscription.findOneAndUpdate.mockResolvedValue({ userId: "user_1" });
            ...
            await paymentService.handleWebhook(Buffer.from(""), "sig");
            expect(Subscription.findOneAndUpdate).toHaveBeenCalledWith({ stripeSubscriptionId: "sub_stripe_1" }, expect.objectContaining({
                status: "cancelled",
        ...
        it("should handle checkout.session.completed and topup credits", async () => {
            ...
            AuditLog.create.mockResolvedValue({});
            await paymentService.handleWebhook(Buffer.from(""), "sig");
            expect(creditsService.topup).toHaveBeenCalledWith("507f1f77bcf86cd799439011", 100, "plan_topup");
            expect(Payment.create).toHaveBeenCalled();
        });
    ...
    describe("getUserPayments", () => {
        it("should return paginated payments for a user", async () => {
            ...
            const mockTotal = 1;
            Payment.find.mockImplementationOnce(() => ({
                sort: jest.fn().mockReturnThis(),
                skip: jest.fn().mockReturnThis(),
                limit: jest.fn().mockReturnThis(),
                populate: jest.fn().mockResolvedValue(mockPaymentsData),
            }));
            Payment.countDocuments.mockResolvedValue(mockTotal);
    ...
    describe("getPaymentById", () => {
        it("should return a single payment if owned by user", async () => {
            ...
            Payment.findOne.mockImplementationOnce(() => ({
