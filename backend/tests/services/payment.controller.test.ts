...
describe("PaymentController Unit Tests", () => {
  ...
  describe("getUserPayments", () => {
    it("should return paginated payments for the authenticated user", async () => {
      ...
      await paymentController.getUserPayments(mockRequest as any, mockResponse as Response, nextFunction);
    ...
    it("should pass errors to next()", async () => {
      mockRequest = { user: { _id: { toString: () => "user123" } } } as any;
      (paymentService.getUserPayments as jest.Mock).mockRejectedValue(new Error("DB Error"));
  ...
  describe("getPaymentDetail", () => {
    it("should return a single payment detail for the authenticated user", async () => {
      ...
      await paymentController.getPaymentDetail(mockRequest as any, mockResponse as Response, nextFunction);
    ...
    it("should pass errors to next()", async () => {
      mockRequest = { user: { _id: { toString: () => "user123" } }, params: { id: "pay123" } } as any;
  ...
  describe("downloadInvoice", () => {
    it("should generate and return a PDF invoice", async () => {
      ...
      (paymentService.generateInvoicePdf as jest.Mock).mockResolvedValue(mockPdfBuffer);

      // Mock res.setHeader and res.send
      mockResponse.setHeader = jest.fn();
      mockResponse.send = jest.fn();

      await paymentController.downloadInvoice(mockRequest as any, mockResponse as Response, nextFunction);
    ...
    it("should pass errors to next()", async () => {
      mockRequest = { user: { _id: { toString: () => "user123" } }, params: { id: "pay123" } } as any;
      (paymentService.generateInvoicePdf as jest.Mock).mockRejectedValue(new Error("PDF generation failed"));
