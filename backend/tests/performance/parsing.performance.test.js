...
const createMockFile = (mimetype, size, content) => ({
    fieldname: "contract",
...
describe("Performance: File Validation", () => {
    ...
    test("should validate DOCX file under 10ms", () => {
        const file = createMockFile("application/vnd.openxmlformats-officedocument.wordprocessingml.document", 1024, "test");
        const start = Date.now();
        for (let i = 0; i < 100; i++) {
...
//# sourceMappingURL=parsing.performance.test.js.map