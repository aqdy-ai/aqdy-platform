...
const createMockContract = (clauses) => Array.from({ length: clauses }, (_, idx) => ({
    clauseNumber: idx + 1,
    clauseText: `Clause text ${idx + 1}`,
    clauseType: idx % 2 === 0 ? "termination" : "liability",
}));
const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
function setupMocks({ clauseCount }) {
    ...
    mockClassify.mockImplementation(async (clauseText) => {
        await sleep(30);
        return {
...
describe("Performance Suite: Contract Pipeline", () => {
    test("should process 10 clauses concurrently within target time", async () => {
        ...
        const result = await orchestratorService.run("perf-contract", "perf-user", "Dummy contract text for performance test.", "en");
    ...
    test("should process 30 clauses concurrently and keep latency predictable", async () => {
        ...
        const runStart = performance.now();
        const result = await orchestratorService.run("perf-contract-large", "perf-user-large", "Dummy contract text for larger performance test.", "en");
        const runDuration = performance.now() - runStart;
        expect(result.executiveSummary.totalClauses).toBe(30);
        expect(runDuration).toBeLessThan(2200);
    });
});
//# sourceMappingURL=pipeline.performance.test.js.map