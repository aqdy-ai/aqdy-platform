...
describe("Credits domain and account credits endpoint", () => {
    ...
    test("estimateCost applies weighted analysis cost formula", () => {
        ...
        const expected = creditsService.calculateAnalysisCost(inputTokens, outputTokens);
    ...
    test("calculateChatCost uses flat CHAT_CREDIT_COST per message", () => {
        expect(creditsService.calculateChatCost(3000, 1000)).toBe(env.CHAT_CREDIT_COST);
    });
    test("deduct throws InsufficientCreditsError for low balance", async () => {
        ...
        await expect(creditsService.deduct(String(user._id), 20)).rejects.toBeInstanceOf(InsufficientCreditsError);
    });
