...
const mockContractFindChain = (contracts) => {
...
const mockAnalysisFindChain = (analyses) => {
...
describe("ContractHistoryService - getContractList", () => {
    ...
    test("should filter by filename", async () => {
        ...
        expect(mockContractFind).toHaveBeenCalledWith(expect.objectContaining({
    ...
    test("should sort by riskLevel descending", async () => {
        ...
        const mockAnalyses = [
            {
                _id: validAnalysisId,
                contractId: mockContracts[0]._id,
                executiveSummary: { overallRisk: "low" },
            },
            {
                _id: new mongoose.Types.ObjectId().toString(),
                contractId: mockContracts[1]._id,
    ...
    test("should only return contracts for the authenticated user", async () => {
        ...
        mockAnalysisFindChain([]);
        await service.getContractList(validUserId);
        expect(mockContractFind).toHaveBeenCalledWith(expect.objectContaining({ userId: validUserId }));
    });
});
describe("ContractHistoryService - getContractDetail", () => {
    ...
    test("should enforce ownership - query includes userId", async () => {
        ...
        expect(mockContractFindOne).toHaveBeenCalledWith(expect.objectContaining({ userId: validUserId }));
    });
    test("should exclude soft deleted contracts - query includes deletedAt: null", async () => {
        mockContractFindOne.mockResolvedValue(null);
        await service.getContractDetail(validContractId, validUserId);
        expect(mockContractFindOne).toHaveBeenCalledWith(expect.objectContaining({ deletedAt: null }));
    });
});
describe("ContractHistoryService - softDeleteContract", () => {
    ...
    test("should set deletedAt on soft delete", async () => {
        ...
        expect(mockContractFindOneAndUpdate).toHaveBeenCalledWith(expect.objectContaining({
            _id: validContractId,
            userId: validUserId,
            deletedAt: null,
        }), expect.objectContaining({ deletedAt: expect.any(Date) }), expect.anything());
    });
    ...
    test("should hide soft deleted contract from list queries", async () => {
        ...
        mockAnalysisFindChain([]);
        await service.getContractList(validUserId);
        expect(mockContractFind).toHaveBeenCalledWith(expect.objectContaining({ deletedAt: null }));
    });
});
//# sourceMappingURL=contractHistory.service.test.js.map