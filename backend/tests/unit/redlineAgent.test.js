...
describe("RedlineAgent", () => {
    ...
    test("should throw an error for empty clause text", async () => {
        await expect(agent.generate("", "high", "limitation-of-liability", "en")).rejects.toThrow("Clause text is empty");
        await expect(agent.generate("   ", "high", "limitation-of-liability", "en")).rejects.toThrow("Clause text is empty");
    });
    test("should generate redline suggestions successfully (without safer alternative)", async () => {
        ...
        const result = await agent.generate("Liability shall be unlimited.", "high", "limitation-of-liability", "en");
        // 2. Assertions
    ...
    test("should boost confidence score when safer alternative is provided", async () => {
        ...
        const result = await agent.generate("Liability shall be unlimited.", "critical", "limitation-of-liability", "en", "Liability capped at 100k USD.");
        // 2. Assertions
        expect(mockInvoke).toHaveBeenCalledTimes(1);
    ...
    test("should throw error if LLM returns invalid JSON", async () => {
        ...
        await expect(agent.generate("Clause text", "medium", "confidentiality", "en")).rejects.toThrow();
    });
...
//# sourceMappingURL=redlineAgent.test.js.map