...
const { ContractExportService } =
  await import("../../src/services/contractExport.service.js");
...
describe("ContractExportService - getExportData", () => {
  let service: InstanceType<typeof ContractExportService>;
  ...
  test("should only include non-deleted contracts", async () => {
    ...
    expect(mockContractFind).toHaveBeenCalledWith(
      expect.objectContaining({ userId: validUserId, deletedAt: null }),
    );
  });
});
