...
import { detectLanguage, normalizeArabicText, convertArabicNumerals, chunkContract, safeParseJSON, mergeExtractionResults, } from "../../src/utils/text.utils.js";
// ── detectLanguage ───────────────────────────────
describe("detectLanguage", () => {
    ...
    test("should return 'en' for English text", () => {
        expect(detectLanguage("This is a standard English employment contract.")).toBe("en");
    });
    test("should return 'ar' for Arabic text", () => {
        expect(detectLanguage("هذا عقد عمل باللغة العربية ويحتوي على بنود متعددة.")).toBe("ar");
    });
    test("should return 'ar' for mixed text with majority Arabic", () => {
        expect(detectLanguage("عقد عمل بين شركة TechCorp وبين الموظف أحمد حسن في القاهرة")).toBe("ar");
    });
    test("should return 'en' for mixed text with majority English", () => {
        expect(detectLanguage("This contract is between TechCorp and Ahmed Hassan, located in القاهرة.")).toBe("en");
    });
...
describe("convertArabicNumerals", () => {
    ...
    test("should handle mixed text with Arabic-Indic numerals", () => {
        expect(convertArabicNumerals("المادة ٣: فترة الاختبار")).toBe("المادة 3: فترة الاختبار");
    });
    test("should leave Western digits unchanged", () => {
        expect(convertArabicNumerals("Article 3: Probation")).toBe("Article 3: Probation");
    });
    test("should handle mixed numeral systems", () => {
        expect(convertArabicNumerals("١٢ months or 12 months")).toBe("12 months or 12 months");
    });
});
// ── chunkContract ────────────────────────────────
describe("chunkContract", () => {
    ...
    test("should handle text without paragraph breaks", () => {
        ...
        const longParagraph = "Sentence one. Sentence two. Sentence three. Sentence four. Sentence five.";
    ...
    test("should handle large contract with many paragraphs", () => {
        ...
        const paragraphs = Array.from({ length: 50 }, (_, i) => `Paragraph ${i + 1}: ${"x".repeat(80)}`);
...
describe("safeParseJSON", () => {
    ...
    test("should throw for completely invalid content", () => {
        expect(() => safeParseJSON("This is not JSON at all")).toThrow("Failed to parse JSON");
    });
