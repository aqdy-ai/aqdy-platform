...
describe('Confidence Scoring & Hallucination Detection', () => {
  ...
  describe('Edge Cases: Unusual Contract Language', () => {
    test('should handle very short contract text', async () => {
      jest.spyOn(orchestrator as any, 'run').mockResolvedValue({
        executiveSummary: {
          overallRisk: 'unknown',
          totalClauses: 0,
          riskyClausesCount: 0,
        ...
        clauseAnalysis: [],
        extractionMeta: { modelUsed: 'gpt-4o', usedFallback: false, chunkCount: 1 },
        durationMs: 100,
      });
    ...
    test('should handle mixed Arabic/English contract', async () => {
      ...
      const result = await orchestrator.run(
        'contract_123',
        'user_123',
        'Liability clause مسؤولية. Payment terms شروط الدفع.',
        'en',
      );
  ...
  describe('Edge Cases: Non-Standard Clause Structures', () => {
    test('all clauses should have confidence score', async () => {
      ...
      const result = await orchestrator.run(
        'contract_123', 'user_123', 'Some contract text', 'en'
      );
      ...
      const lowConfidenceClauses = result.clauseAnalysis.filter(
        c => c.confidence < 0.6
      );
      ...
      const noKBClauses = result.clauseAnalysis.filter(
        c => c.sourceFromKB === null
      );
