...
const { registerUser, loginUser, logoutUser, refreshTokens, verifyAccessToken, loginWithGoogle, } = await import("../../src/services/auth.service.js");
describe("Auth Service", () => {
    ...
    test("loginWithGoogle creates a new Google user", async () => {
        ...
        mockSave.mockResolvedValue(undefined);
        const result = await loginWithGoogle("mock-google-token-john");
    ...
    test("loginWithGoogle rejects invalid mock token", async () => {
        process.env.NODE_ENV = "test";
        await expect(loginWithGoogle("bad-token")).rejects.toThrow(AppError);
    });
