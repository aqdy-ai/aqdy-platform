...
const { RiskClassifierAgent } = await import("../../src/agents/riskClassifier.agent.js");
// ── Helpers ──────────────────────────────────────
...
describe("RiskClassifierAgent", () => {
    ...
    test("should throw an error for empty clause text", async () => {
        await expect(agent.classify("", "probation", "en")).rejects.toThrow("Clause text is empty");
        await expect(agent.classify("   ", "probation", "en")).rejects.toThrow("Clause text is empty");
    });
    test("should classify clause successfully with high-score KB match (RAG context)", async () => {
        ...
        const result = await agent.classify("The probation period is six months.", "probation", "en");
        // 3. Assertions
    ...
    test("should ignore KB match if score is below threshold", async () => {
        ...
        const result = await agent.classify("The salary will be paid late.", "payment", "en");
        // 3. Assertions
    ...
    test("should handle case where Pinecone returns no results", async () => {
        ...
        const result = await agent.classify("Standard clause text here.", "general", "en");
        // 3. Assertions
    ...
    test("should degrade gracefully and classify without RAG if Pinecone throws an error", async () => {
        ...
        const result = await agent.classify("The employee shall work 80 hours a week without overtime.", "overtime", "en");
        // 3. Assertions
    ...
    test("should throw error if LLM returns invalid JSON", async () => {
        ...
        await expect(agent.classify("Some clause", "general", "en")).rejects.toThrow();
    });
});
//# sourceMappingURL=riskClassifierAgent.test.js.map