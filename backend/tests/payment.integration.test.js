...
beforeAll(() => {
    ...
    jest
        ...
        .mockImplementation(async (subId) => {
...
function buildStripeEvent(type, data) {
    const payloadObj = {
...
test("full checkout flow creates subscription and tops up credits", async () => {
    ...
    await paymentService.fulfillSubscription(fakeSession);
...
test("processes all relevant Stripe webhook events", async () => {
    ...
    const { payload: csPayload, signature: csSig } = buildStripeEvent("checkout.session.completed", csCompleted);
    ...
    const { payload: invPayload, signature: invSig } = buildStripeEvent("invoice.paid", invoicePaid);
    ...
    const { payload: failPayload, signature: failSig } = buildStripeEvent("invoice.payment_failed", invoiceFailed);
    ...
    const { payload: delPayload, signature: delSig } = buildStripeEvent("customer.subscription.deleted", delEvent);
...
test("exhausted free plan credits trigger 402", async () => {
    ...
    await expect(creditsService.deduct(testUserId, 1, { reason: "analysis_deduction" })).rejects.toThrow(InsufficientCreditsError);
});
