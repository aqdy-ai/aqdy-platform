...
jest.unstable_mockModule("../../src/middlewares/auth.middleware.js", () => ({
    ...
    requireRole: (..._roles) => (_req, _res, next) => {
        next();
    },
    requirePermission: (_section, _action) => (_req, _res, next) => {
        next();
    },
...
describe("Upload Route Security Integration", () => {
    ...
    test("should accept and sanitize clean uploaded document", async () => {
        ...
        expect(mockSaveContract).toHaveBeenCalledWith(expect.objectContaining({
            text: "This contract binds both parties.", // HTML and script tag stripped
        }));
    });
});
//# sourceMappingURL=pipeline.security.integration.test.js.map