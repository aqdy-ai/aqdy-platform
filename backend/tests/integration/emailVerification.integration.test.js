 ...
 */
const createUnverifiedUser = async (email, name = "Test User", password = "StrongPass123!") => {
...
describe("Email Verification Integration Tests", () => {
    ...
    test("Unverified user created directly has isEmailVerified: false and emailVerificationToken", async () => {
        const { user, verificationToken } = await createUnverifiedUser("verify-test@example.com", "Verify Tester");
    ...
    test("Unverified user is blocked from protected routes with 403 Forbidden", async () => {
        ...
        const { token: access } = await createUnverifiedUser("unverified@example.com", "Unverified User");
    ...
    test("POST /api/auth/verify-email verifies email with valid token", async () => {
        ...
        const { verificationToken } = await createUnverifiedUser("to-verify@example.com", "User To Verify");
    ...
    test("POST /api/auth/resend-verification enforces rate limits", async () => {
        ...
        const { token: access } = await createUnverifiedUser("resend-user@example.com", "Resend User");
    ...
    test("Admin can manually verify a user's email via PATCH /api/admin/accounts/:id", async () => {
        ...
        const { user: targetUser } = await createUnverifiedUser("admin-target@example.com", "Admin Target User");
...
//# sourceMappingURL=emailVerification.integration.test.js.map