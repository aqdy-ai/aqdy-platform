...
const REDLINE_RESPONSES = {
  "liability-cap-en": {
    suggestedText:
      "In no event shall either party's total aggregate liability exceed the total fees paid under this Agreement in the twelve (12) months preceding the claim.",
    explanation: {
  ...
  "non-compete-limited-en": {
    suggestedText:
      "The Employee agrees not to compete with the Company for a period of twelve (12) months following termination within the Arab Republic of Egypt.",
    explanation: {
  ...
  "liability-cap-ar": {
    suggestedText:
      "لا يجوز في أي حال من الأحوال أن يتجاوز إجمالي مسؤولية أي من الطرفين إجمالي الرسوم المدفوعة بموجب هذا العقد خلال الاثني عشر (١٢) شهراً السابقة للمطالبة.",
    explanation: {
      en: "Introduces a standard liability cap to protect both parties from unlimited financial exposure.",
      ar: "يضع حداً أقصى للمسؤولية لحماية الطرفين من التعرض لمخاطر مالية غير محدودة.",
    },
  ...
  "non-compete-limited-ar": {
    suggestedText:
      "يتعهد الموظف بعدم المنافسة لمدة ١٢ شهراً بعد انتهاء العقد وذلك داخل نطاق جمهورية مصر العربية.",
    explanation: {
...
const testCases = [
  {
    originalText:
      "Service Provider shall be liable for all damages, losses, and injuries of any kind without limit.",
    riskLevel: "critical",
  ...
  {
    originalText:
      "Employee shall not work for any competitor for 5 years after contract ends worldwide.",
    riskLevel: "high",
  ...
  {
    originalText:
      "يكون مقدم الخدمة مسؤولاً عن جميع الأضرار والخسائر والإصابات من أي نوع دون حد أقصى.",
    riskLevel: "critical",
    clauseType: "liability",
    lang: "ar",
  ...
  {
    originalText:
      "يتعهد الموظف بعدم العمل لدى أي منافس لمدة ٥ سنوات بعد انتهاء العقد في أي مكان في العالم.",
    riskLevel: "high",
    clauseType: "non_compete",
    lang: "ar",
...
describe("RedlineAgent — Integration Tests for Suggestion Quality", () => {
  ...
  test.each(testCases)(
    ...
    async ({ originalText, riskLevel, clauseType, lang, mockKey }) => {
      ...
      expect(result.suggestedText).toBe(
        REDLINE_RESPONSES[mockKey].suggestedText,
      );
      expect(result.explanation.en).toBe(
        REDLINE_RESPONSES[mockKey].explanation.en,
      );
  ...
  test("should handle LLM failure gracefully", async () => {
    ...
    await expect(
      agent.generate("text", "high", "liability", "en"),
    ).rejects.toThrow("All LLM providers failed. Please try again");
  }, 10000);
