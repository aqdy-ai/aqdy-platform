...
function buildMockSession(userId, planId, planSlug, subscriptionId = "sub_test_" + Date.now(), customerId = "cus_test_" + Date.now()) {
    return {
...
function buildWebhookEvent(type, data, secret) {
...
describe("PaymentService.createCheckoutSession", () => {
    it("resolves plan by slug and returns a checkout URL", async () => {
        ...
        const mockCreate = jest
            ...
            .mockResolvedValue(stripeResponse({
        ...
        const result = await paymentService.createCheckoutSession(String(user._id), plan.slug);
    ...
    it("resolves plan by ObjectId when slug lookup fails", async () => {
        ...
        const result = await paymentService.createCheckoutSession(String(user._id), String(plan._id));
        expect(result.url).toBeDefined();
        jest.restoreAllMocks();
    });
    it("throws 404 when user does not exist", async () => {
        await expect(paymentService.createCheckoutSession(new mongoose.Types.ObjectId().toString(), "premium")).rejects.toMatchObject({ statusCode: 404 });
    });
    it("throws 400 for inactive plan", async () => {
        const user = await createTestUser();
        const plan = await createTestPlan({ isActive: false });
        await expect(paymentService.createCheckoutSession(String(user._id), plan.slug)).rejects.toMatchObject({ statusCode: 400 });
    });
    it("throws 400 for free plan checkout", async () => {
        ...
        await expect(paymentService.createCheckoutSession(String(user._id), "free")).rejects.toMatchObject({ statusCode: 400 });
...
describe("PaymentService.fulfillSubscription (post-payment activation)", () => {
    it("activates subscription, updates user.plan, and tops up credits", async () => {
        ...
        const mockSession = buildMockSession(String(user._id), String(plan._id), plan.slug, subId, customerId);
    ...
    it("is idempotent — calling twice handles balance tracking safely based on database logic", async () => {
        ...
        const mockSession = buildMockSession(String(user._id), String(plan._id), plan.slug, subId);
        jest.spyOn(stripe.subscriptions, "retrieve").mockResolvedValue(stripeResponse({
            id: subId,
            current_period_start: Math.floor(Date.now() / 1000),
            current_period_end: Math.floor(Date.now() / 1000) + 2592000,
...
describe("PaymentService.handleWebhook", () => {
    it("throws 400 on invalid webhook signature", async () => {
        ...
        await expect(paymentService.handleWebhook(payload, "invalid_signature")).rejects.toMatchObject({ statusCode: 400 });
    });
    it("is idempotent — skips duplicate webhook events", async () => {
        ...
        const fulfillSpy = jest
            .spyOn(paymentService, "fulfillSubscription")
