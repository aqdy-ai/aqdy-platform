...
// ── Shared fixtures ───────────────────────────────────────────────────────────
const ENGLISH_CONTRACT_TEXT = "This is a sample employment contract between Employer and Employee. " +
...
const ARABIC_CONTRACT_TEXT = "يلتزم الموظف بأداء المهام المنوطة به وفقاً لتوجيهات الإدارة. " +
...
function makeDocxFile(overrides = {}) {
    return {
        fieldname: "contract",
...
describe("Upload → Extract → Store Pipeline", () => {
    ...
    describe("Step 1 — File parsing (language detection & validation)", () => {
        ...
        test("pdfService rejects files over 10MB", () => {
            ...
            expect(() => pdfService.validateFile(file)).toThrow("File size exceeds 10MB");
        });
    ...
    describe("Step 3 & 4 — ExtractorAgent called and clauses stored", () => {
        test("triggerAnalysis() invokes the LLM with the correct text", async () => {
            ...
            await analysisService.triggerAnalysis(MOCK_CONTRACT_ID, "user_test", ENGLISH_CONTRACT_TEXT, "en");
        ...
        test("triggerAnalysis() passes Arabic language to the extractor correctly", async () => {
            ...
            await analysisService.triggerAnalysis(MOCK_CONTRACT_ID, "user_ar", ARABIC_CONTRACT_TEXT, "ar");
            expect(mockInvoke).toHaveBeenCalledTimes(1);
            expect(mockAnalysisSave).toHaveBeenCalledTimes(1);
        });
    });
    ...
    describe("Full pipeline — spying on parsePdf / parseDocx", () => {
        test("PDF pipeline: parse → save contract → extract → persist analysis", async () => {
            ...
            expect(mockContractSave).toHaveBeenCalledTimes(1);
            await analysisService.triggerAnalysis(String(contract._id), "user_test", parsed.text, parsed.language);
            expect(mockInvoke).toHaveBeenCalledTimes(1);
            expect(mockAnalysisSave).toHaveBeenCalledTimes(1);
            expect(mockAuditSave).toHaveBeenCalled();
        ...
        test("Arabic PDF pipeline: parse (Arabic text) → save → extract → persist", async () => {
            ...
            await analysisService.triggerAnalysis(String(contract._id), "user_ar", parsed.text, parsed.language);
            expect(mockContractSave).toHaveBeenCalledTimes(1);
            expect(mockInvoke).toHaveBeenCalledTimes(1);
            expect(mockAnalysisSave).toHaveBeenCalledTimes(1);
            parseSpy.mockRestore();
    ...
    describe("Error handling", () => {
        test("LLM failure — triggerAnalysis() does not throw, does not persist analysis", async () => {
            ...
            await expect(analysisService.triggerAnalysis(MOCK_CONTRACT_ID, "user_test", "FAILING_CONTRACT_TEXT_TO_BYPASS_CACHE", "en")).resolves.toBeUndefined();
...
//# sourceMappingURL=upload-pipeline.integration.test.js.map