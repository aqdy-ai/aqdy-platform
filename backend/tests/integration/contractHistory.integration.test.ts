...
async function createContract(userId: string, overrides = {}) {
...
async function createAnalysis(contractId: string, userId: string, overallRisk = 'high') {
...
describe('Filters', () => {
    ...
    test('should filter by status analyzed', async () => {
        const contract1 = await createContract(userId, { filename: 'analyzed.pdf' });
