...
let authToken;
let userId;
const analysisRequest = (payload) => request(app)
...
describe("Analysis API Integration", () => {
    ...
    it("should handle 5 concurrent analysis requests successfully", async () => {
        const requests = Array.from({ length: 5 }).map(() => analysisRequest({ contractId: realContractId, userId }));
