...
function makeClause(clauseType, riskLevel, overrides = {}) {
    return {
...
function makeAnalysisData(contractId, overrides = {}) {
    return {
...
describe("RiskAnalysis Versioning", () => {
    ...
    describe("AnalysisService.getAnalysisVersionsByContractId", () => {
        ...
        test("returns all versions in descending order", async () => {
            ...
            await service.saveAnalysis(makeAnalysisData(contractIdA));
            await service.saveAnalysis(makeAnalysisData(contractIdA));
            const versions = await service.getAnalysisVersionsByContractId(contractIdA.toString());
        ...
        test("returns empty array when no analyses exist", async () => {
            const versions = await service.getAnalysisVersionsByContractId(new mongoose.Types.ObjectId().toString());
        ...
        test("each version includes overallRisk from executiveSummary", async () => {
            ...
            expect(versions[0].executiveSummary.overallRisk).toBe("medium");
        });
        test("does not include clauseAnalysis in the list response", async () => {
            await service.saveAnalysis(makeAnalysisData(contractIdA));
            const versions = await service.getAnalysisVersionsByContractId(contractIdA.toString());
            expect(versions[0].clauseAnalysis).toBeUndefined();
        });
    });
    ...
    describe("AnalysisService.getAnalysisById", () => {
        ...
        test("returns null for a non-existent ID", async () => {
            const result = await service.getAnalysisById(new mongoose.Types.ObjectId().toString());
            expect(result).toBeNull();
        });
        test("includes diffSummary in v2+ detail", async () => {
            await service.saveAnalysis(makeAnalysisData(contractIdA));
            const v2 = await service.saveAnalysis(makeAnalysisData(contractIdA, {
                clauseAnalysis: [
    ...
    describe("AnalysisService.generateDiffSummary", () => {
        ...
        test("handles empty previous clauses", () => {
            const diff = service.generateDiffSummary([], [makeClause("termination", "high")], 1);
            expect(diff.totalChanged).toBe(0);
        });
        test("handles empty current clauses", () => {
            const diff = service.generateDiffSummary([makeClause("termination", "low")], [], 1);
            expect(diff.totalChanged).toBe(0);
        });
        test("correctly reports multiple changed clauses", () => {
            ...
            const escalated = diff.changedClauses.filter((c) => c.direction === "escalated");
            const deEscalated = diff.changedClauses.filter((c) => c.direction === "de-escalated");
    ...
    describe("Model — indexes", () => {
        test("compound unique index exists on (contractId, version)", async () => {
            ...
            const hasCompoundIndex = indexes.some((idx) => idx.key.contractId === 1 &&
                idx.key.version === -1 &&
                idx.unique === true);
        ...
        test("index on (contractId, createdAt) exists", async () => {
            await RiskAnalysis.init();
            const indexes = await RiskAnalysis.listIndexes();
            const hasIndex = indexes.some((idx) => idx.key.contractId === 1 && idx.key.createdAt === -1);
            expect(hasIndex).toBe(true);
        });
        test("index on (userId, createdAt) exists", async () => {
            await RiskAnalysis.init();
            const indexes = await RiskAnalysis.listIndexes();
            const hasIndex = indexes.some((idx) => idx.key.userId === 1 && idx.key.createdAt === -1);
            expect(hasIndex).toBe(true);
        });
    });
});
//# sourceMappingURL=riskAnalysis.versioning.test.js.map