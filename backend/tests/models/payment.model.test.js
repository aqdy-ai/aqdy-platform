...
describe("Payment Model Test", () => {
    ...
    test("Test 4: Assert that the indexes are successfully defined on the compiled model", async () => {
        ...
        // Compound index
        const hasCompoundIndex = indexes.some((index) => index.key.userId === 1 && index.key.createdAt === -1);
        // ProviderTxId index
        const hasProviderTxIdIndex = indexes.some((index) => index.key.providerTxId === 1 && index.unique === true);
        expect(hasIdIndex).toBe(true);
        expect(hasCompoundIndex).toBe(true);
        expect(hasProviderTxIdIndex).toBe(true);
    });
});
//# sourceMappingURL=payment.model.test.js.map